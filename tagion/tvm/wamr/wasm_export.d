...
module tagion.tvm.wamr.wasm_export;
...
alias get_module_inst = wasm_runtime_get_module_inst;

extern (D) auto validate_app_addr(T0, T1)(auto ref T0 offset, auto ref T1 size)
{
    return wasm_runtime_validate_app_addr(module_inst, offset, size);
}

extern (D) auto validate_app_str_addr(T)(auto ref T offset)
{
    return wasm_runtime_validate_app_str_addr(module_inst, offset);
}

extern (D) auto addr_app_to_native(T)(auto ref T offset)
{
    return wasm_runtime_addr_app_to_native(module_inst, offset);
}

extern (D) auto addr_native_to_app(T)(auto ref T ptr)
{
    return wasm_runtime_addr_native_to_app(module_inst, ptr);
}

extern (D) auto module_malloc(T0, T1)(auto ref T0 size, auto ref T1 p_native_addr)
{
    return wasm_runtime_module_malloc(module_inst, size, p_native_addr);
}

extern (D) auto module_free(T)(auto ref T offset)
{
    return wasm_runtime_module_free(module_inst, offset);
}
...
import tagion.tvm.wamr.wasm_runtime_common :WASMModuleInstanceCommon;
alias wasm_module_inst_t = WASMModuleInstanceCommon*;

/* Function instance */
alias WASMFunctionInstanceCommon = void;
alias wasm_function_inst_t = void*;
 ...
 */
package_type_t get_package_type (const(ubyte)* buf, uint size);

/**
 * It is a callback for WAMR providing by embedding to load a module file
 * into a buffer
 */
alias module_reader = bool function (
 ...
 */
void wasm_runtime_set_module_reader (
 ...
 */
bool wasm_runtime_register_module (
    const(char)* module_name,
 ...
 */
wasm_module_t wasm_runtime_find_module_registered (const(char)* module_name);
 ...
 * @return return WASM module loaded, NULL if failed
 */
wasm_module_t wasm_runtime_load (
    const(ubyte)* buf,
    uint size,
    char* error_buf,
    uint error_buf_size);
 ...
 * @return return WASM module loaded, NULL if failed
 */
wasm_module_t wasm_runtime_load_from_sections (
    wasm_section_list_t section_list,
    bool is_aot,
    char* error_buf,
    uint error_buf_size);
 ...
 */
void wasm_runtime_unload (wasm_module_t module_);

void wasm_runtime_set_wasi_args (
    wasm_module_t module_,
    const(char)** dir_list,
    uint dir_count,
...
bool wasm_runtime_is_wasi_mode (wasm_module_inst_t module_inst);

wasm_function_inst_t wasm_runtime_lookup_wasi_start_function (
 ...
 */
wasm_function_inst_t wasm_runtime_lookup_function (
 ...
 *   the exception info.
 */
bool wasm_application_execute_main (
    wasm_module_inst_t module_inst,
    int argc,
    char** argv);
 ...
 */
bool wasm_application_execute_func (
    wasm_module_inst_t module_inst,
    const(char)* name,
    int argc,
    char** argv);
 ...
 */
void wasm_runtime_set_exception (
    wasm_module_inst_t module_inst,
 ...
 */
void wasm_runtime_set_custom_data (
    wasm_module_inst_t module_inst,
    void* custom_data);
/**
 ...
 */
void* wasm_runtime_get_custom_data (wasm_module_inst_t module_inst);
 ...
 */
int wasm_runtime_module_malloc (
    wasm_module_inst_t module_inst,
    uint size,
    void** p_native_addr);
 ...
 */
void wasm_runtime_module_free (wasm_module_inst_t module_inst, int ptr);
 ...
 *         Return non-zero if success, zero if failed.
 */
int wasm_runtime_module_dup_data (
    wasm_module_inst_t module_inst,
    const(char)* src,
    uint size);
 ...
 *         be thrown.
 */
bool wasm_runtime_validate_app_addr (
    wasm_module_inst_t module_inst,
    int app_offset,
    uint size);
 ...
 *         be thrown.
 */
bool wasm_runtime_validate_native_addr (
    wasm_module_inst_t module_inst,
    void* native_ptr,
    uint size);
 ...
 */
void* wasm_runtime_addr_app_to_native (
    wasm_module_inst_t module_inst,
    int app_offset);
 ...
 */
int wasm_runtime_addr_native_to_app (
    wasm_module_inst_t module_inst,
    void* native_ptr);
...
/**
  ...
  * @return true if success, false otherwise
  */
bool wasm_runtime_register_natives (
    const(char)* module_name,
    NativeSymbol* native_symbols,
    uint n_native_symbols);
 ...
 */
bool wasm_runtime_register_natives_raw (
    const(char)* module_name,
    NativeSymbol* native_symbols,
    uint n_native_symbols);
 ...
 */
void* wasm_runtime_get_function_attachment (wasm_exec_env_t exec_env);
 ...
 */
void wasm_runtime_set_user_data (wasm_exec_env_t exec_env, void* user_data);
/**
 ...
 */
void* wasm_runtime_get_user_data (wasm_exec_env_t exec_env);
